...
class File extends SplFileObject
{
    ...
    {
        if ($this->getSize()/1024 > (int) $size) {
            $this->error = '文件大小不匹配,最大上传支持:'.$size.'/KB，当前上传文件大小为:'.($this->getSize()/1024).'/KB';
            return false;
        }
        return true;
    }
    ...
    private function error($errorNo)
    {
        switch ($errorNo) {
            case 1:
            case 2:
                $this->error = '上传文件大小超过最大值';
                break;
            case 3:
                $this->error = '只上传文件的一部分';
                break;
            case 4:
                $this->error = '没有要上传的文件';
                break;
            case 6:
                $this->error = '找不到上传临时目录';
                break;
            case 7:
                $this->error = '文件写入错误';
                break;
